...
# Colab TensorFlow CPU vs GPU timing demo
...
results = {}
for dev in devices:
    # If GPU requested but not available, skip
    if dev.startswith('/GPU') and not tf.config.list_physical_devices('GPU'):
        print(f"Skipping {dev}: no GPU found.")
        continue
    ...
    with tf.device(dev):
        ...
        results[dev] = elapsed
        print(f"Elapsed time on {dev}: {elapsed:.2f} seconds")

print("\nSummary of training times:")
for dev, t in results.items():
    print(f"  {dev:6s} → {t:.2f}s")
